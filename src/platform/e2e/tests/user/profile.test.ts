...
describe("Profile Page @user", function () {
  ...
  it("should show password fields in security tab", async function () {
    await profilePage.navigateToProfile();
    await profilePage.clickSecurityTab();
    expect(await profilePage.isDisplayed(ProfilePage["CURRENT_PASSWORD"])).to.be.true;
    expect(await profilePage.isDisplayed(ProfilePage["NEW_PASSWORD"])).to.be.true;
    expect(await profilePage.isDisplayed(ProfilePage["CONFIRM_PASSWORD"])).to.be.true;
  });
