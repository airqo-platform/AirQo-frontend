...
describe("Reset Password @auth", function () {
  ...
  it("should show invalid link or error for missing token @smoke", async function () {
    ...
    const hasError = await page.isDisplayed(By.css(".text-destructive, [class*='error'], [class*='alert']"), 3);
  ...
  it("should show invalid link for fake token", async function () {
    ...
    await new Promise((r) => setTimeout(r, 3000));
    const isInvalid = await page.isInvalidLinkState();
    const hasForm = await page.isDisplayed(ResetPasswordPage["PASSWORD_INPUT"], 3);
    const hasError = await page.isDisplayed(By.css(".text-destructive, [class*='error']"), 3);
