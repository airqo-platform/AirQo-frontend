...
const RolePermissionsEditor = ({ onRefresh }) => {
  ...
  const roleId = params?.role_id || params?.id || null;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredPermissions.map((permission) => {
                ...
                return (
                  <div
                    key={permission._id}
                    className={`relative rounded-lg border-2 transition-all duration-200 cursor-pointer hover:shadow-md ${isSelected ? 'border-primary bg-primary/10' : 'border-gray-200 bg-white hover:border-gray-300'}`}
                    onClick={() =>
                  ...
                  >
                    <div className="p-4">
                      <div className="flex items-center space-x-3">
                        <div
                          className={`relative w-5 h-5 rounded border-2 transition-all duration-200 ${isSelected ? 'bg-primary border-primary' : 'border-gray-300 hover:border-gray-400'}`}
                        >
                          {isSelected && (
                            <svg
                        ...
                        <div className="flex-1 min-w-0">
                          <h3
                            className={`text-sm font-medium transition-colors duration-200 ${isSelected ? 'text-primary' : 'text-gray-900'}`}
                          >
                            {formatPermissionName(permission.permission)}
                          </h3>
                          <p
                            className={`text-xs mt-1 transition-colors duration-200 ${isSelected ? 'text-primary' : 'text-gray-500'}`}
                          >
                            {permission.description}
                          </p>
                        </div>
