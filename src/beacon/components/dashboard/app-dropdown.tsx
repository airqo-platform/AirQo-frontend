...
const AppDropdown: React.FC<AppDropdownProps> = ({ className = '' }) => {
    ...
    return (
        <DropdownMenu onOpenChange={(open) => { if (!open) setShowQRCode(false); }}>
            <DropdownMenuTrigger
                className={`p-2 rounded-full transition hover:bg-muted focus:outline-none ${className}`}
            ...
            <DropdownMenuContent
                className="w-auto border-none shadow-lg z-[10000] bg-white p-3"
                align="end"
            >
                {!showQRCode ? (
                    ...
                    <div className="p-2 w-[280px]">
                        <div className="flex items-center justify-between mb-4">
                            <button
                                ...
                                className="p-2 rounded-full hover:bg-muted focus:outline-none transition-colors"
                                aria-label="Back to apps"
                            >
                                <AqArrowNarrowLeft className="w-5 h-5 text-muted-foreground" />
                            </button>
                            <h3 className="text-base font-semibold">Get AirQo Mobile App</h3>
                            <div className="w-9" aria-hidden="true" />
                        </div>

                        <div className="flex justify-center mb-4">
                            <div className="w-48 h-48 bg-white overflow-hidden rounded-2xl border border-gray-200 shadow-sm flex items-center justify-center relative">
                                <Image
                                    src="/images/AQR.jpeg"
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-black text-white text-xs font-medium rounded-lg hover:bg-gray-800 transition-colors focus:outline-none"
