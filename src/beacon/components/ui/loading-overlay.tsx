...
const LoadingOverlay: React.FC<LoadingOverlayProps> = ({
  ...
  return (
    <div
      className={cn(
        'fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-background/80 transition-opacity duration-300 ease-in-out',
    ...
    >
      {hasCopy ? (
        <div
          className={cn(
            'flex flex-col items-center gap-4 rounded-2xl border border-border bg-card/90 px-8 py-6 text-center shadow-md transition-opacity duration-300 ease-in-out',
            isFadingOut ? 'opacity-0' : 'opacity-100'
          )}
        >
          <div
