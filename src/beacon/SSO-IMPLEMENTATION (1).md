# AirQo Single Sign-On (SSO) Implementation Guide
...
> **CRITICAL**: The `NEXTAUTH_SECRET` MUST be the same across all apps. If it differs, JWTs signed by one app cannot be decoded by another, and SSO will fail silently.
...
> **Key**: `session.user._id = token._id || token.id` — The `|| token.id` fallback is essential for SSO. If App A stores `_id` but App B's JWT only has `id`, the fallback ensures `_id` is still populated.
...
### Important: Cookie Domain on Logout
...
> **Why `window.location.href`?** Using `router.push()` does client-side navigation which doesn't fully clear in-memory NextAuth state. `window.location.href` forces a full page reload, ensuring all session state is cleared.
...
| Symptom | Cause | Fix |
|---|---|---|
| SSO doesn't work (login required on both) | Cookie domain not set or mismatched | Verify `NEXTAUTH_COOKIE_DOMAIN=.airqo.local` in both apps |
