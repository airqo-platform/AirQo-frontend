...
import { ProfileReadinessPanel } from "@/components/diagnostics/ProfileReadinessPanel";
...
export default function DeviceProfileDetailPage() {
  ...
  // Diagnostic readiness is recomputed server-side from the saved profile, so refresh it after every edit.
  ...
  const fetchReadiness = useCallback(async () => {
    ...
    try {
      ...
      setReadiness(await diagnosticsService.getProfileReadiness(profileId));
    } catch (err: any) {
      console.error("Error fetching diagnostic readiness:", err);
      setReadinessError(err?.message || "Failed to check diagnostic readiness.");
    } finally {
      ...
      <Card className="border border-gray-200 shadow-xs overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {readiness && (
                      <button
                        ...
                        title="Open diagnostic readiness"
                      >
                        <Stethoscope className="w-3.5 h-3.5" />
                        {readiness.diagnosable ? "Diagnostics Ready" : "Not Diagnosable"}
                      </button>
                    )}
                  </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
              ...
              {Object.entries(profile.meta_data).map(([k, v]) => (
                <span key={k} className="px-2 py-0.5 rounded-md bg-slate-100 text-[11px] font-mono text-gray-700 border">
                  <span className="text-gray-400">{k}:</span> {typeof v === "object" && v !== null ? JSON.stringify(v) : String(v)}
                </span>
              ))}
            </div>
          )}
        ...
        <CardContent className="pt-4 pb-6 space-y-4">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid grid-cols-5 h-9 bg-slate-100 p-1 text-xs">
              ...
              <TabsTrigger value="diagnostics" className="text-xs gap-1.5 flex items-center justify-center">
                ...
                {readiness &&
                  (readiness.diagnosable ? (
                    ...
                    <span className="w-1.5 h-1.5 rounded-full bg-rose-500 ml-0.5 shrink-0" />
                  ))}
              </TabsTrigger>
              <TabsTrigger value="json" className="text-xs gap-1.5 flex items-center justify-center">
                <Code className="w-3.5 h-3.5 text-slate-600" />
                <span>5. JSON Schema</span>
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="diagnostics" className="pt-4">
              <ProfileReadinessPanel
                ...
                policyOverrides={profile.meta_data?.diagnostics}
                onEditOverrides={() => setHeaderModalOpen(true)}
              />
            </TabsContent>
