 ...
 * Mirrors the exact theme schema, tokens, and storage keys used in Beacon and Nexus.
 */
...
export const GROUP_THEME_STORAGE_PREFIX = 'theme_group_';
export const THEME_SCRIPT_ID = 'theme-script';
...
export function saveThemeToStorage(theme: ThemeData, groupId?: string): void {
  ...
  try {
    ...
    // Also update general theme as fallback/default
...
export function applyThemeImmediately(theme: ThemeData): void {
  ...
  // Derive lighter and darker shades matching Beacon and Nexus
 ...
 * Ensures zero-flicker on initial page load when opened from Nexus/Beacon or refreshed.
 */
  ...
  return `
    (function() {
      try {
        ...
        // 1. Try to find the active group from Redux persist or localStorage
        try {
          ...
          if (userRaw) {
            ...
            if (userPersist.activeGroup) {
              ...
              if (activeGroup) {
                activeGroupId = activeGroup._id || activeGroup.id;
              }
            }
          }

          if (!activeGroupId) {
            activeGroupId = localStorage.getItem('activeGroup') ||
                            localStorage.getItem('selectedGroup') ||
                            localStorage.getItem('beacon_active_group');
          }

          if (activeGroupId) {
            var groupKey = 'theme_group_' + activeGroupId;
