...
import { FlowStep } from '../claim-device-modal';


const MethodSelectStep = ({ onSelect }: { onSelect: (step: FlowStep) => void }) => (
    ...
    <div className="grid grid-cols-1 gap-4">
      {[
        ...
        },
        {
          ...
          border: 'hover:border-violet-500 dark:hover:border-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/20',
          title: 'Import from Cohort',
          desc: 'Enter a Cohort ID to prefill devices.',
        },
      ].map(({ step, icon, iconBg, border, title, desc }) => (
        <button
          key={step}
        ...
        >
          <div className={`p-2 ${iconBg} rounded-full transition-colors mr-4 shrink-0`}>{icon}</div>
          <div>
