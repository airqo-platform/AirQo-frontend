...
interface ReusableSelectInputProps extends Omit<React.SelectHTMLAttributes<HTMLSelectElement>, "onChange"> {
  label?: string
  error?: string
  description?: string
...
const ReusableSelectInput: React.FC<ReusableSelectInputProps> = ({
  label,
  error,
  description,
  containerClassName = "",
  ...
  return (
    <div ref={containerRef} className={cn("flex flex-col", containerClassName)}>
      ...
      <div className="relative">
        ...
        >
          <span className={cn("truncate", !selectedItem && "text-gray-400 dark:text-gray-500")}>
            {selectedItem ? selectedItem.label : placeholder}
          </span>
          <svg
            className={cn(
              "w-5 h-5 ml-2 transition-transform duration-200 flex-shrink-0",
          ...
          >
            ...
            >
              {items.length > 0 ? (
                ...
                <li className="px-4 py-2.5 text-sm text-gray-500 dark:text-gray-400">No options available</li>
              )}
            </ul>
          </div>
        )}
      ...
      {error && (
        <div id={`${buttonId}-error`} className="mt-1.5 flex items-center text-xs text-red-600 dark:text-red-400">
      ...
      {!error && description && (
        <div id={`${buttonId}-description`} className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
          {description}
        </div>
      )}
    </div>
