...
import { Mark, searchBarShortcut, searchBarShortcutHint, searchBarPosition, searchContextByPaths, hideSearchBarWithNoSearchContext, useAllContextsWithNoSearchContext, } from "@easyops-cn/docusaurus-search-local/dist/client/client/utils/proxiedGenerated";
...
export default function SearchBar({ handleSearchBarToggle, }) {
    ...
    const effectiveVersionUrlRef = useRef(versionUrl);
    ...
    useEffect(() => {
        effectiveVersionUrlRef.current = versionUrl;
    }, [versionUrl]);
    ...
    const loadIndex = useCallback(async () => {
        ...
        let autoComplete;
        try {
            autoComplete = await fetchAutoCompleteJS();
            try {
                ...
                if (!fallbackSucceeded && versionUrl !== "/docs/") {
                    try {
            ...
            console.error("[docs-search] Failed to initialize local search index", error);
            indexStateMap.current.delete(searchContext);
            setLoading(false);
            return;
        }
        ...
        search.current = autoComplete(searchBarRef.current, {
        ...
        }, [
            {
                source: async (input, callback) => {
                    const result = await searchByWorker(effectiveVersionUrlRef.current, searchContext, input, searchResultLimits);
                    callback(result);
                },
                templates: {
    ...
    dir="ltr">
      <input placeholder={translate({
            ...
            message: "Search",
            description: "The ARIA label and placeholder for search button",
        })} className={`navbar__search-input ${styles.searchInput}`} onMouseEnter={onInputMouseEnter} onFocus={onInputFocus} onBlur={onInputBlur} onChange={onInputChange} ref={searchBarRef} value={inputValue}/>
