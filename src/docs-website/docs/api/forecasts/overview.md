...
The same scope metadata (`scope`, `sites_count`, `sites_with_forecasts_count`, `site_names`) is also included when you pass `grid_id`/`cohort_id` as a query parameter on the base endpoint — the path-parameter form above is just an alternative way to address the same request.
...
| `aqi.aqi_category` | string | AQI category name (see AQI Categories below) |
...
| `met.wind_from_direction` | number | Wind direction in degrees |
| `met.wind_direction_compass` | string | Wind direction as a compass point (e.g. `"NE"`, `"SW"`) |
...
Forecast results are cached server-side for about an hour. Forecasts themselves are regenerated once per day, so caching responses on your end for a similar window gives a good balance between API call volume and data freshness. See [Best Practices →](../reference/best-practices.md) for caching guidance.
