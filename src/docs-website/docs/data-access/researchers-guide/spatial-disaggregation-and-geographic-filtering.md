...
- **Programmatically via the API**: Call the [Find the nearest sites to a coordinate](../../api/reference/metadata.md#find-the-nearest-sites-to-a-coordinate) endpoint with a `latitude`/`longitude`/`radius` to get back active sites sorted by distance — useful for scripting a batch of locations rather than looking them up one at a time in the app.
