---
sidebar_position: 3
sidebar_label: Resetting Your Password
---

# Resetting Your Password

If you forget your password, you can reset it by email.

---

## Request a Reset Link

1. Go to the [AirQo Nexus login page](https://analytics.airqo.net/user/login).
2. Select **Forgot Password?**
3. Enter the email address associated with your account.
4. Select **Submit**.
5. Check your inbox for a password reset email.

:::tip Can’t find the email?
Check your spam or junk folder.
:::

---

## Set a New Password

1. Open the reset link in the email.
2. On the reset page, enter a **new password** and **confirm password**.
3. Select **Reset Password**.
4. After a few seconds you are redirected to the [login page](https://analytics.airqo.net/user/login), where you can sign in with your new password.

:::caution Reset links expire
If your link is invalid or expired, return to the login page and request a new one.
:::

---

## Related Guides

- [Creating an Account](./creating-an-account.md)
- [Logging In](./logging-in.md)
