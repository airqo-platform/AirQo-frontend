...
**Data availability.** A download always succeeds as a file, but the file may contain only metadata if no measurement readings exist for the chosen locations, date range, and pollutants. In that case you get a metadata-only file (location names, coordinates, device info) and the file name gets a `-metadata` suffix so you can tell it apart.

**90-day batch limit.** Each export can cover at most **90 days**. Date ranges longer than that show a warning in the sidebar and are blocked at download time. To export a full year, make four 90-day (or smaller) exports and stitch them together afterward.
...
| Date range too large                                                                  | Sidebar warning **Download Limit Notice**; at download: toast **"Date Range Too Large"** / "Please split this export into batches of 90 days or fewer to avoid backend timeouts." | Use 90 days or fewer. For annual data, make four separate exports and combine them afterward.                                                                                 |
...
- **Batch annual data.** Four 90-day exports are faster and more reliable than one giant pull. Stitch them together in a spreadsheet afterward.
