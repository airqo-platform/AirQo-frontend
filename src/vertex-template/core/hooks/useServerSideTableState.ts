...
export const useServerSideTableState = ({
    ...
    // Sync local state when URL params change (e.g., back/forward)
    useEffect(() => {
        setPagination((prev) =>
            JSON.stringify(prev) === JSON.stringify(initialPagination) ? prev : initialPagination
        );
        setSearchTerm((prev) => (prev === initialSearchTerm ? prev : initialSearchTerm));
        setSorting((prev) =>
            JSON.stringify(prev) === JSON.stringify(initialSorting) ? prev : initialSorting
        );
    }, [initialPagination, initialSearchTerm, initialSorting]);
