...
class _TimeWindowSheetState extends State<TimeWindowSheet> {
  ...
  void initState() {
    super.initState();
    _wdArrive = widget.place.weekdayWindow?.arrive ??
        const TimeOfDay(hour: 7, minute: 0);
    _wdLeave = widget.place.weekdayWindow?.leave ??
        const TimeOfDay(hour: 17, minute: 0);
    _weArrive = widget.place.weekendWindow?.arrive ??
        const TimeOfDay(hour: 9, minute: 0);
    _weLeave = widget.place.weekendWindow?.leave ??
  ...
  void _setArrive(TimeOfDay t) =>
      setState(() => _isWeekdays ? _wdArrive = t : _weArrive = t);
  void _setLeave(TimeOfDay t) =>
      setState(() => _isWeekdays ? _wdLeave = t : _weLeave = t);

  bool get _absentCurrentSegment =>
      _isWeekdays ? _absentWeekdays : _absentWeekends;

  String get _hint {
    if (_isWeekdays && _absentWeekdays) {
      return 'Not scheduled on weekdays · ${widget.place.displayName}';
    }
    if (!_isWeekdays && _absentWeekends) {
      return 'Not scheduled on weekends · ${widget.place.displayName}';
    }
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('When are you usually here?',
                          style: TextStyle(
                              fontSize: 18,
                      ...
                      Text(
                          'Set your typical schedule to track exposure accurately.',
                          style: TextStyle(fontSize: 13, color: txt2)),
                      const SizedBox(height: 20),
                      _SegmentControl(
                          selected: _isWeekdays ? 0 : 1,
                          labels: const ['Weekdays', 'Weekends'],
                          onChanged: (i) =>
                              setState(() => _isWeekdays = i == 0)),
                          ...
                          child: Column(
                            children: [
                              _TimeField(
                                  ...
                                  isDark: isDark,
                                  onTap: () =>
                                      _pick(context, _arrive, _setArrive)),
                              ...
                              _TimeField(
                                  ...
                                  isDark: isDark,
                                  onTap: () =>
                                      _pick(context, _leave, _setLeave)),
                            ],
                          ),
                        ),
                      ),
  ...
  Widget _handle(Color txt2) => Padding(
        ...
        child: Center(
            child: Container(
                ...
                height: 4,
                decoration: BoxDecoration(
                    color: txt2.withValues(alpha: 0.4),
