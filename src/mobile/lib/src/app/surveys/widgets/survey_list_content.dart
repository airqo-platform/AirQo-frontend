...
/// Scrollable research surveys list — shared by the Learn tab and standalone page.
class SurveyListContent extends StatefulWidget {
  const SurveyListContent({super.key});

  @override
  State<SurveyListContent> createState() => _SurveyListContentState();
}

class _SurveyListContentState extends State<SurveyListContent> {
  @override
  void initState() {
    super.initState();
    _loadSurveys();
                      ...
                      child: Column(
                        children: state.surveys.map((survey) {
                          ...
                          return Padding(
                            ...
                            child: SurveyCard(
                              survey: survey,
                              showProgress: userResponse != null &&
                                  userResponse.isInProgress,
                              completionPercentage: userResponse
                                  ?.getCompletionPercentage(
                                      survey.questions.length),
                              onTap: () => _navigateToSurveyDetail(
                                survey,
                                userResponse,
                              ),
                            ),
  ...
  Widget _buildResearchImpactCard(SurveysLoaded state) {
    ...
    final completedCount =
        state.userResponses.where((r) => r.isCompleted).length;
    final totalResponses = completedCount * 1000;
    final researchProgress = (completedCount * 8.3).clamp(0.0, 100.0);
      ...
      builder: (context, constraints) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  AppColors.primaryColor.withValues(alpha: isDark ? 0.4 : 0.2),
            ),
            boxShadow: [
              BoxShadow(
  ...
  Widget _buildStatsCard(SurveysLoaded state) {
    ...
    final inProgressCount =
        state.userResponses.where((r) => r.isInProgress).length;
