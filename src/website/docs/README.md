# AirQo Documentation

**Last Updated**: February 13, 2026

---

## 📚 Documentation Index

### Architecture & Structure

**[APP_STRUCTURE.md](./APP_STRUCTURE.md)** ⭐ **NEW**  
Comprehensive guide to the website's application structure, route organization, and multi-section architecture. Essential reading for understanding the codebase.

### SEO & Interactive Components

**[AIRQO_SEO_GUIDE.md](./AIRQO_SEO_GUIDE.md)**  
Consolidated SEO strategy and implementation roadmap. Start here for all SEO-related work.

### Features & Integrations

**[AIR_QUALITY_BILLBOARD.md](./AIR_QUALITY_BILLBOARD.md)**  
Interactive air quality billboard component documentation.

**[COOKIE_CONSENT_IMPLEMENTATION.md](./COOKIE_CONSENT_IMPLEMENTATION.md)**  
Cookie consent system implementation guide.

**[COOKIE_CONSENT_QUICKSTART.md](./COOKIE_CONSENT_QUICKSTART.md)**  
Quick reference for cookie consent.

**[GOOGLE_TRANSLATE_INTEGRATION.md](./GOOGLE_TRANSLATE_INTEGRATION.md)**  
Google Translate widget integration for multilingual support.

**[METADATA_IMPROVEMENTS.md](./METADATA_IMPROVEMENTS.md)**  
SEO metadata enhancements and structured data.

---

## Quick Reference

**Goal**: 3x African traffic by Q2 2026  
**Network**: 400+ monitors across 14 countries  
**Key Pages**: `/billboard/interactive`, `/solutions/network-coverage`

**New Sections**:

- `/packages` - Open source packages and developer tools
- `/blog` - News, insights, and stories (coming soon)

---

**For SEO work**: See [AIRQO_SEO_GUIDE.md](./AIRQO_SEO_GUIDE.md)  
**For app structure**: See [APP_STRUCTURE.md](./APP_STRUCTURE.md)
