...
describe('Air Quality pages (end-to-end)', function () {
  ...
  before(async () => {
    ...
    for (let round = 0; round < 6; round++) {
      ...
      await driver.executeScript(`
        ...
        const tileRoot = title?.closest('div.rounded-md');
        [...(tileRoot?.querySelectorAll('button') || [])].find(b => b.getAttribute('aria-label') === 'Chart actions')?.click();
      `);
  ...
  after(async () => {
    ...
    const noise =
      /Download the React DevTools|Extra attributes from the server|preloaded using link preload/;
  ...
  const waitFor = async (by: By, timeout = 30) => {
    ...
    await driver.wait(
      until.elementIsVisible(await find(by, 5)),
      timeout * 1000
    );
  };
  ...
  const waitForDialogReady = async () => {
    try {
      ...
      await waitFor(
        By.xpath(
          "//div[@role='dialog']//input[@placeholder='e.g. PM2.5 levels across Kampala']"
        ),
        45
      );
      await waitFor(
        By.xpath(
          "//div[@role='dialog']//input[@placeholder='e.g. Hourly PM2.5 for selected sites']"
        ),
        45
      );
    } catch (error) {
  ...
  it('Rankings: renders live leaderboard with legend, filters, summary cards and table', async () => {
    ...
    await waitFor(
      By.xpath("//h1[contains(normalize-space(), 'Air Quality Rankings')]"),
      60
    );

    // Page-level AQI legend with ranges (static � no toggle button)
    expect(await visible(By.xpath("//*[normalize-space()='Hazardous']"))).to.be
      .true;
    ...
    expect(
      await visible(
        By.xpath("//*[@role='radio' and normalize-space()='Live rankings']")
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath(
          "//*[@role='radio' and normalize-space()='Historical comparison']"
        )
      )
    ).to.be.true;
    ...
    expect(
      await visible(
        By.xpath("//*[@role='radio' and normalize-space()='Country']")
      )
    ).to.be.true;
    expect(
      await visible(By.xpath("//*[@role='radio' and normalize-space()='City']"))
    ).to.be.true;
    expect(
      await visible(
    ...
    expect(await visible(By.xpath("//*[normalize-space()='Cleanest air']"))).to
      .be.true;
    expect(await visible(By.xpath("//*[normalize-space()='Locations ranked']")))
      .to.be.true;

    // Leaderboard table with rows
    await waitFor(
      By.xpath("//h2[contains(normalize-space(), 'Live rankings')]"),
      30
    );
    const rows = await driver.findElements(
      By.xpath("//table[.//th[normalize-space()='Rank']]//tbody/tr")
    );
  ...
  it('Rankings: level/sort/limit filters update the table', async () => {
    ...
    await driver.sleep(2000);
    const cityRows = await driver.findElements(
      By.xpath("//table[.//th[normalize-space()='Rank']]//tbody/tr")
    );
    ...
    await click(
      By.xpath("//*[@role='radio' and normalize-space()='Cleanest first']")
    );
    await driver.sleep(2000);
    const firstCell = await text(
      By.xpath("//table[.//th[normalize-space()='Rank']]//tbody/tr[1]/td[2]")
    );
    ...
    await driver
      .findElement(
        By.xpath(
          "//select[@aria-label='Number of entries']/option[normalize-space()='Top 10']"
        )
      )
      .click();
    await driver.sleep(2000);
    const top10Rows = await driver.findElements(
      By.xpath("//table[.//th[normalize-space()='Rank']]//tbody/tr")
    );
  ...
  it('Rankings: historical comparison tab renders year filters, chart and table', async () => {
    ...
    await waitFor(
      By.xpath(
        "//button[normalize-space()='2024' or normalize-space()='2026']"
      ),
      45
    );
    expect(await visible(By.xpath("//button[normalize-space()='2024']"))).to.be
      .true;
    expect(await visible(By.xpath("//button[normalize-space()='2026']"))).to.be
      .true;
    await waitFor(
      By.xpath("//h2[contains(normalize-space(), 'Year-by-year comparison')]"),
      45
    );
    // Chart title is a CardTitle (h3), not h2
    expect(
      await visible(
        By.xpath("//h3[contains(normalize-space(), 'PM2.5 trends by year')]")
      )
    ).to.be.true;

    const table = await driver.findElements(
      By.xpath("//table[.//th[normalize-space()='Location']]//tbody/tr")
    );
  ...
  it('Analytics: page renders with view switcher, legend and add-chart affordance', async () => {
    ...
    // The workspace renders once charts exist; the empty state shows the CTA
    const hasCharts = await visible(
      By.xpath("//*[@role='radio' and normalize-space()='Focused']"),
      5
    );
    if (hasCharts) {
      expect(
        await visible(
          By.xpath("//*[@role='radio' and normalize-space()='Charts']")
        )
      ).to.be.true;
      expect(
        await visible(
          By.xpath("//*[@role='radio' and normalize-space()='Table']")
        )
      ).to.be.true;
      expect(await visible(By.xpath("//*[normalize-space()='Hazardous']"))).to
        .be.true;
    } else {
      expect(
        await visible(
          By.xpath("//*[contains(normalize-space(), 'No charts yet')]")
        )
      ).to.be.true;
    }
  ...
  it('Analytics: add chart dialog exposes every configuration setting', async () => {
    ...
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//button[normalize-space()='PM2.5']")
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//button[normalize-space()='Daily']")
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//button[normalize-space()='Line']")
      )
    ).to.be.true;

    // Date range picker
    expect(
      await visible(By.xpath("//div[@role='dialog']//button[contains(., '-')]"))
    ).to.be.true;

    // No dedicated color sections outside the table (colors are opt-in via
    // the picker's Color column, which appears once rows are selected)
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//*[normalize-space()='Series color']")
      )
    ).to.be.false;
    expect(
      await visible(
    ...
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//*[normalize-space()='Show legend']")
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//*[normalize-space()='Show grid']")
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//*[normalize-space()='Show tooltips']")
      )
    ).to.be.true;
    ...
    expect(
      await visible(
        By.xpath(
          "//div[@role='dialog']//*[@role='radio' and normalize-space()='Sites']"
        )
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath(
          "//div[@role='dialog']//*[@role='radio' and normalize-space()='Devices']"
        )
      )
    ).to.be.true;

    // Save disabled until title + site selected
    const saveBtn = await find(
      By.xpath("//div[@role='dialog']//button[normalize-space()='Add chart']")
    );
  ...
  it('Analytics: create a chart with two sites and verify the tile renders', async () => {
    ...
    await click(
      By.xpath(
        "//div[@role='dialog']//*[@role='option' and normalize-space()='PM10']"
      )
    );
    expect(
      await visible(
        By.xpath("//div[@role='dialog']//button[normalize-space()='PM10']")
      )
    ).to.be.true;

    // Switch back to PM2.5 so the saved chart supports the forecast toggle
    await click(
      By.xpath("//div[@role='dialog']//button[normalize-space()='PM10']")
    );
    await click(
      By.xpath(
        "//div[@role='dialog']//*[@role='option' and normalize-space()='PM2.5']"
      )
    );
    ...
    await click(
      By.xpath(
        "//div[@role='dialog']//*[@role='option' and normalize-space()='Bar']"
      )
    );
    ...
    await waitFor(
      By.xpath(
        "//div[@role='dialog']//input[starts-with(@aria-label,'Select item')]"
      ),
      30
    );
    ...
    // Color column appears once rows are selected: one color input per
    // selected site (inside the table rows)
    await waitFor(
      By.xpath(
        "//div[@role='dialog']//input[starts-with(@aria-label,'Color for ')]"
      ),
      15
    );
    const locationColorInputs = await driver.findElements(
      By.xpath(
        "//div[@role='dialog']//input[starts-with(@aria-label,'Color for ')]"
      )
    );
    expect(locationColorInputs.length).to.be.greaterThan(0);
    ...
    expect(await visible(By.xpath(`//h3[normalize-space()='${CHART_TITLE}']`)))
      .to.be.true;

    // Header exposes Edit directly and the menu holds the remaining actions
    await waitFor(
      By.xpath(
        `//h3[normalize-space()='${CHART_TITLE}']/ancestor::div[2]//button[normalize-space()='Edit']`
      ),
      15
    );
    await click(
      By.xpath(
        `//h3[normalize-space()='${CHART_TITLE}']/ancestor::div[2]//button[@aria-label='Chart actions']`
      )
    );
    // Menu items are div[role=menuitem], not buttons (custom dropdown)
    expect(
      await visible(
        By.xpath("//*[@role='menuitem' and normalize-space()='Rename']")
      )
    ).to.be.true;
    expect(
      await visible(
        By.xpath("//*[@role='menuitem' and normalize-space()='Duplicate']")
      )
    ).to.be.true;
    expect(
      await visible(
    ...
    expect(
      await visible(
        By.xpath("//*[@role='menuitem' and normalize-space()='Delete chart']")
      )
    ).to.be.true;
    ...
    await waitFor(
      By.xpath(`//h3[contains(normalize-space(), '${CHART_TITLE} (Copy)')]`),
      30
    );
    const menuStillOpen = await visible(
      By.xpath("//*[@role='menuitem' and normalize-space()='Rename']"),
      3
    );
    expect(menuStillOpen).to.be.false;

    // Edit dialog opens from the header Edit button and pre-selects sites
    await click(
      By.xpath(
        `//h3[contains(normalize-space(), '${CHART_TITLE} (Copy)')]/ancestor::div[2]//button[normalize-space()='Edit']`
      )
    );
    await waitFor(
      By.xpath(
        "//div[@role='dialog']//h2[normalize-space()='Edit chart configuration']"
      ),
      30
    );
    // Pre-selected sites render one location-color row each (the "Selected"
    // chips block was removed — the table and color rows show the selection)
    await waitFor(
      By.xpath(
        "//div[@role='dialog']//input[starts-with(@aria-label,'Color for ')]"
      ),
    ...
    // Forecast toggle is part of the chart card; summary card sits below
    await driver.executeScript(
      `window.scrollTo(0, document.body.scrollHeight)`
    );
    const forecastToggle = await find(
      By.xpath("//button[@role='switch' and @aria-label='Forecast']"),
      15
    );
    await forecastToggle.click();
    await waitFor(By.xpath("//*[normalize-space()='Forecast summary']"), 20);
    expect(await visible(By.xpath("//*[normalize-space()='PM2.5 only']"))).to.be
      .true;
  ...
  it('Analytics: inline title edit persists from the chart header', async () => {
    await driver.get(`${BASE}/user/air-quality/analytics`);
    await waitFor(
      By.xpath(`//h3[contains(normalize-space(), '${CHART_TITLE}')]`),
      60
    );

    await click(
      By.xpath(
        `//h3[contains(normalize-space(), '${CHART_TITLE}')]/ancestor::div[2]//button[@aria-label='Chart actions']`
      )
    );
    await click(
      By.xpath("//*[@role='menuitem' and normalize-space()='Rename']")
    ...
    await waitFor(
      By.xpath(`//h3[normalize-space()='${CHART_TITLE} (edited)']`),
      30
    );
  ...
  it('Analytics: view modes — focused workspace, charts overview, comparison table', async () => {
    await driver.get(`${BASE}/user/air-quality/analytics`);
    await waitFor(
      By.xpath(`//h3[contains(normalize-space(), '${CHART_TITLE}')]`),
      60
    ...
    // Focused workspace (default) shows the active chart
    expect(
      await visible(
        By.xpath(`//h3[contains(normalize-space(), '${CHART_TITLE}')]`)
      )
    ...
    // Compare table
    await click(By.xpath("//*[@role='radio' and normalize-space()='Table']"));
    await waitFor(
      By.xpath("//h2[contains(normalize-space(), 'Location comparison')]"),
      30
    );
    expect(
      await visible(By.xpath("//select[@aria-label='Comparison pollutant']"))
    ).to.be.true;

    // Table rows exist (sites from the chart)
    const rows = await driver.findElements(
      By.xpath("//table[.//th[normalize-space()='PM2.5']]//tbody/tr")
    );
    expect(rows.length).to.be.greaterThan(0);
    ...
    await click(
      By.xpath(
        "//table[.//th[normalize-space()='PM2.5']]//th[.//span[normalize-space()='PM2.5']]"
      )
    );
    ...
    await click(By.xpath("//*[@role='radio' and normalize-space()='Focused']"));
    await waitFor(
      By.xpath(`//h3[contains(normalize-space(), '${CHART_TITLE}')]`),
      15
    );
  ...
  it('Analytics: delete chart with inline confirmation', async () => {
    ...
    // Arm the inline delete confirmation via the chart menu (scoped to the
    // active chart's card � executeScript approach proven by the cleanup script)
    const armDelete = `
    ...
    await driver.sleep(700);
    await driver.executeScript(`
      const del = [...document.querySelectorAll('[role="menuitem"]')].find(el => el.innerText.trim() === 'Delete chart');
      del && del.click();
    `);

    // Menu closes and the card shows an inline confirm strip
    await waitFor(By.xpath("//*[normalize-space()='Delete this chart?']"), 15);
    const menuStillOpen = await visible(
      By.xpath("//*[@role='menuitem' and normalize-space()='Delete chart']"),
      3
    );
    ...
    // Delete for real — the Duplicate step above leaves a second chart with
    // a matching title, so keep deleting until every E2E chart is gone.
    let stillThere = true;
    for (let round = 0; round < 6; round++) {
      await driver.executeScript(armDelete);
      await driver.sleep(700);
      ...
      // Chart disappears (after the last deletion the workspace empties)
      await driver.sleep(4000);
      stillThere = await visible(
        By.xpath(`//h3[contains(normalize-space(), '${CHART_TITLE}')]`),
        3
      );
      if (!stillThere) break;
