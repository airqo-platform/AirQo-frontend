  ...
  e2e:
    ...
    steps:
      ...
      - name: Install e2e dependencies
        working-directory: src/nexus/e2e
        run: yarn install
      ...
      - uses: actions/upload-artifact@v4
        ...
        with:
          name: e2e-report
          path: src/nexus/e2e/reports/
      - uses: actions/upload-artifact@v4
        if: failure()
        with:
          name: e2e-screenshots
          path: src/nexus/e2e/screenshots/
```
...
pipeline {
    ...
    stages {
        stage('E2E Tests') {
            steps {
                sh 'cd src/nexus/e2e && yarn install && yarn test:report'
            }
        }
    }
    post {
        always {
            publishHTML(target: [
                ...
                reportDir: 'src/nexus/e2e/reports',
                reportFiles: '*.html'
            ])
            archiveArtifacts artifacts: 'src/nexus/e2e/screenshots/**', allowEmptyArchive
        }
    }
}
```
