...
### Data Export
- **Data availability check before download** — a new banner (`DataAvailabilityBanner`) checks each selected site against the recent-readings endpoint and shows "Data available / Partial coverage / No readings for the selected period". When no data is available, the primary action changes from "Review & Download" to a disabled "No Data for Selection" with guidance buttons ("Change Filters", "Choose Other Locations").
...
- Files: `src/modules/data-download/` (new hook + banner, edits to page/header/state hook).
...
| Data availability not checked early | ✅ Implemented (banner) |
...
| "Data available / Partial / No readings" indicators | ✅ Implemented |
