...
describe('getEnvironmentAwareUrl', () => {
  ...
  it('maps platform.airqo.net to staging-platform.airqo.net on localhost', () => {
    (globalThis as unknown as Record<string, unknown>).window = {
      location: { hostname: 'localhost', href: 'http://localhost:3000' },
    };
    const result = getEnvironmentAwareUrl('https://platform.airqo.net/api');
    expect(result).toBe('https://staging-platform.airqo.net/api');
  });
