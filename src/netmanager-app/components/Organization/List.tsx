...
export function OrganizationList() {
      ...
      queries: [
        {
          queryKey: ["all-sites"],
          queryFn: async () => {
            // Since we don't have a direct API to get all sites, we'll fetch sites for each group
            const allSites: Site[] = []
        ...
        },
        {
          ...
          queryFn: async () => {
            // Since we don't have a direct API to get all devices, we'll fetch devices for each group
            const allDevices: Device[] = []
            for (const group of groups) {
              try {
                const response = (await devices.getDevicesSummaryApi(
                  "",
                  group.grp_title || "",
                )) as DevicesSummaryResponse
                if (response.devices) {
  ...
  useEffect(() => {
    if (!isLoadingSites && !isLoadingDevices && sitesData && devicesData) {
      ...
      // Process sites
      if (sitesData.sites) {
        sitesData.sites.forEach((site: Site) => {
          // Check site's groups array
          if (site.groups && Array.isArray(site.groups)) {
            site.groups.forEach((groupIdentifier: unknown) => {
              // Try to find a matching group by ID or title
              const matchingGroup = groups.find(
                (g: Group) => g._id === groupIdentifier || g.grp_title === groupIdentifier,
              )
          ...
          if (site.devices && Array.isArray(site.devices)) {
            site.devices.forEach((device: any) => {
              if (device.groups && Array.isArray(device.groups)) {
                ...
                const matchingGroup = groups.find((g: Group) => g._id === device.group || g.grp_title === device.group)
      ...
      if (devicesData.devices) {
        devicesData.devices.forEach((device: Device) => {
          ...
          } else if (device.site && device.site.group) {
            const matchingGroup = groups.find(
              (g: Group) => g._id === device.site.group || g.grp_title === device.site.group,
            )
          ...
          if (device.site && device.site.groups && Array.isArray(device.site.groups)) {
            device.site.groups.forEach((groupIdentifier: unknown) => {
              const matchingGroup = groups.find(
                (g: Group) => g._id === groupIdentifier || g.grp_title === groupIdentifier,
              )
