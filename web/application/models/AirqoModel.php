...
class AirqoModel extends CI_Model
{
	...
	public function get_positions($slug = null) {
		if($slug == null) {
			$query = $this->db->query("SELECT * FROM tbl_career");
			return $query->result_array();
		}
		$query = $this->db->query("SELECT * FROM tbl_career WHERE ca_id = '$slug'");
		return $query->row_array();
	}
	...
	{
		if ($id == null) {
			$query = $this->db->query("SELECT p.*, COALESCE(c.c_name, 'General') AS category
								FROM tbl_posts p
	...
	public function get_post_categories($category = null)
	{
		//get per category
		...
		$query = $this->db->query("SELECT p.*, COALESCE(c.c_name, 'General') AS category
							FROM tbl_posts p
							LEFT JOIN tbl_categories c ON p.p_categories = c.c_id
							WHERE p.p_status = 'published' AND p.p_categories = '$category' ORDER BY p.p_updated DESC LIMIT 10");
		return $query->result_array();
	}
	...
	public function get_categories($id = null) {
		if($id == null){
			$query = $this->db->query("SELECT * FROM tbl_categories WHERE c_status = 'active'");
			return $query->result_array();
		}
		$query = $this->db->query("SELECT * FROM tbl_categories WHERE c_id = '$id' AND c_status = 'active'");
		return $query->row_array();
	}
