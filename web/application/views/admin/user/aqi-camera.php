			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div id="approve-message"></div>
						<form action="" method="post" id="approveModalForm">
							<input type="hidden" id="approveID" name="aqi" />
							<table class="table table-striped">
								<tr>
									<th id="approve-init-message"></th>
								</tr>
							</table>
					</div>
          ...
          <div class="modal fade" id="declineModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div id="decline-message"></div>
						<form action="" method="post" id="declineModalForm">
							<input type="hidden" id="declineID" name="aqi" />
							<table class="table table-striped">
								<tr>
									<th id="decline-init-message"></th>
								</tr>
							</table>
					</div>
					<div class="modal-footer">
                    ...
                    <div class="portlet">
                         ...
                         <div class="portlet box blue">
                              ...
                              <div class="portlet-body">
                                   <table class="table table-responsive table-bordered" id="sample_2">
                                        <thead>
                                             <tr>
                                                  <th>No. </th>
                                                  <th>Node </th>
...
<script>
     $(document).on("click", ".open-approve", function() {
		...
		$('#approveID').val(id);
          console.log(id);
		$('#approve-init-message').html('<div class="alert alert-warning">Are you sure you want to approve ' + name + '</div>');
	});
     
     $(document).on("click", ".open-decline", function() {
		var id = $(this).data('id');
		var name = $(this).data('name');
		$('#declineID').val(id);
		$('#decline-init-message').html('<div class="alert alert-warning">Are you sure you want to decline ' + name + '</div>');
	});
     ...
     $('#approveModalForm').submit(function(e) {
          ...
          $.ajax({
               ...
               success: function(response) {
                    ...
                    // $.LoadingOverlay("hide");
                    if (success == 1) {
                         $('#approve-message').html('<div class="alert alert-success" role="alert">' + response.message + '</div>');
                         setTimeout(function() {
                         ...
                         //unable to sign up
                         $('#approve-message').html('<div class="alert alert-danger" role="alert">' + response.message + '</div>');
                    }
               }
          });
     });
     
     $('#declineModalForm').submit(function(e) {
          e.preventDefault();
          $.ajax({
               type: "POST",
               url: "<?= site_url('a-aqi-decline'); ?>",
               data: new FormData(this),
               contentType: false,
               processData: false,
               cache: false,
               ...
               success: function(response) {
                    ...
                    // $.LoadingOverlay("hide");
                    if (success == 1) {
                         $('#decline-message').html('<div class="alert alert-success" role="alert">' + response.message + '</div>');
                         setTimeout(function() {
                              $('#decline-message').html('');
                              document.location = '<?= current_url(); ?>';
                         }, 2000);
                    } else {
                         //unable to sign up
                         $('#decline-message').html('<div class="alert alert-danger" role="alert">' + response.message + '</div>');
                    }
               }
          });
     });
     ...
     var TableAdvanced = function() {
          var initTable2 = function() {
               ...
               var oTable = table.dataTable({
				...
				"processing": "true",
				"ajax": {
					"url": "<?= site_url('a-load-aqi-camera-table'); ?>",
					"type": "POST",
					"data": {
