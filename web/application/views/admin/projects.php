			...
			<div class="portlet">
				<div class="portlet-title">
					...
					<div class="page-bar">
					  			...
					  			<button class='btn btn-success addMember pull-right' id="Storybtn">+ Add Project</button>
					  			<a  href="<?php echo site_url('Admin/projects');?>" class='btn btn-success addMember1 pull-right' id="Storybtn1"> Back</a>
			        </div>
				</div>
				<style type="text/css">
						.btn-success{
					...
					<div class="kaboom">
						...
						<div id="info">
							...
							<table class="table table-responsive table-bordered" width="100%" id="sample_2">
									...
									<tbody>
										<?php
										  $projects = $this->AdminModel->list_projects();
										$i = 0;
										...
										?>
											...
											<td><p><?php echo word_limiter ($row['p_excerts'], 20);?></p></td>
											<!-- <td><?php echo $row['p_image'];?></td> -->
											<td>
												<?php
												if(!empty($row['p_image'])) {
													echo '
													<img class="img_zero"  src="'. base_url().'assets/images/projects/'.$row['p_image'].'" width="70px" height="70px;" alt="Project Image">
													';
												}else{
													echo '';
												}
											...
											<td>
										        <a href="<?php echo base_url('Admin/projectDetails/');?><?= $row['p_id'];?>" type="submit" class="btn btn-primary" id="actionbtn1">  View </a>
										        <br/>
											    <a data-toggle="modal" data-target=".edit_team" class="btn btn-warning" data-id="<?= $row['p_id']; ?>" id="actionbtn2" >Edit</a>
												<br/>
												<button data-id="<?php echo $row['p_id'];?>" class="btn btn-danger actionbtn3 delete-btn" data-toggle="confirmation" data-placement="left" data-singleton="true" type="submit">Delete</button>
											</td>
										<?php
										  }
										?>
						...
						<form name="form" method="post" action="<?php echo site_url('Admin/addProject');?>" class="addMember_form" enctype="multipart/form-data">

							<div class="form-group col-md-12">
							<b>Title:</b>
								<input type="text" name="p_title" class="form-control" placeholder="Enter project name" required/>
							</div>
							<br/>
							<div class="form-group">
								<b>Simple Description:<br/> <small>Please provide some info </small></b>
								<input type="text" maxlength="100" name="p_excerts" class="form-control" placeholder="maximum 100 characters" required/>
							</div>
							<br/>
							</div>
							...
							<div class="form-group col-md-12">
								<div class="row">
									<p><b class="col col-md-2 right">Add A Project Image:</b></p>
								</div>
						    </div>
								<div class="row">
								 	<div class="form-group col-md-12">
										...
										<div class="col-md-6">
											<div class="input-group">
												...
												<label class="btn btn-primary btn-upload" for="inputImage1" title="Upload image file">
							                      <input class="btn btn-default" id="inputImage1" name="p_image" type="file" accept="image/*">
							...
							<div class="form-group pro">
								<button type="submit" class="btn btn-primary " title="Click to add a project" name="submit" id="submit"> Create Project </button>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body"> <!-- GIVE THE TABS DIFFERENT ID FROM THE PREVIOUS ONES 2 AVOID CONFLICT -->
					<form name="form" method="post" action="<?php echo site_url('Admin/editProject');?>"  enctype="multipart/form-data">
						 <input class="form-control" type="hidden" id="p_id" name="p_id" placeholder="" />
							<div class="form-group col-md-12">
								 Title:
								<input type="text" name="p_title" id="pjct_title" class="form-control"  required/>
							</div>
							<br/>
							<div class="form-group col-md-12">
								<b>Simple Description:<br/> <small>Please provide some info </small></b>
								<input type="text" maxlength="100" name="p_excerts" id="p_excerts" class="form-control" placeholder="maximum 100 characters" required/>
							</div>
							<br/>
							</div>
							Content:
							...
							<div class="form-group col-md-12">
									<div class="col-md-6">
										...
										<label class="btn btn-primary btn-upload" for="p_image" title="Upload image file">
												<input class="btn btn-default" id="p_image" name="p_image" type="file" accept="image/*" >
												<span class="docs-tooltip" data-toggle="tooltip" title="Select Image">
								...
								<script>
									var uploadField = document.getElementById("p_image");

										uploadField.onchange = function() {
											if(this.files[0].size > 1007200){
												alert(" The Image File is Too Big! Please Choose Another One");
												$('#error-modal').modal('show');
												this.value = "";
											};
...
$('.edit_team').on('show.bs.modal', function (e) {
	...
	$.ajax({
		type : 'post',
		url  : '<?= site_url("Admin/editProjectDetails"); ?>',
		data : {
		p_id : id
		},
		cache : false,
...
// SCRIPT FOR THE DELETE function
		...
		$('.delete-btn').on('confirmed.bs.confirmation', function(){
		    ...
		    $.ajax({
		        type : 'post',
		        url  : '<?= site_url("Admin/deleteProject"); ?>',
		        data : {
		        	p_id : id
		        },
		        success : function(data){
