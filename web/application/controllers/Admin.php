	...
	public function registerAdministrator()
	{
		...
		if ($this->form_validation->run() === false) {
			...
			if ($uploaded_image != null) {
				...
				$image_name = "adm_" . date('YmdHis') . rand(0000, 9999) . ".jpg";
				imagejpeg($dst_r, './assets/images/admins/' . $image_name, $jpeg_quality);
				...
				if ($send) {
					...
					$this->session->set_flashdata("error", "Oops! something went wrong,Couldn't Create Account.!");
					redirect('a-administrators');
				}
			} else {
				$this->session->set_flashdata("error", "No image added");
	...
	public function addMember()
	{
		if (isset($this->session->userdata['loggedin']['adminname'])) {
			...
			if ($this->form_validation->run() === false) {
				...
				if ($send) {
					$this->session->set_flashdata("msg", "A Team member has been added Succesfully.!!");
					redirect('Admin/team');
				} else {
					$this->session->set_flashdata("error", "Oops! something went wrong,Please Contact sytem Developer.!");
					redirect('Admin/team');
	...
	public function createAppNode()
	{
		if (isset($this->session->userdata['loggedin']['adminname'])) {
			...
			if ($this->form_validation->run() === false) {
				...
				$check_channel = $this->AdminModel->check_channel_id();
				if ($check_channel) {
					$send = $this->AdminModel->create_app_node($data);
					if ($send) {
						$this->session->set_flashdata("msg", "Information saved Succesfully.!!");
						redirect('Admin/appNodes');
					} else {
						$this->session->set_flashdata("error", "Something went wrong, please contact system Developer!");
						redirect('Admin/appNodes');
					}
				} else {
					$this->session->set_flashdata("error_id", "The channel ID provided already exists, Please provide a valid one");
					redirect('Admin/appNodes');
				}
			}
		} else {
	...
	public function editAppNodes()
	{
		if (isset($this->session->userdata['loggedin']['adminname'])) {
			...
			if ($this->form_validation->run() === false) {
				...
				$an_id   = $this->input->post('an_id');
				$send    = $this->AdminModel->update_nodes($data, $an_id);

				if ($send) {
					$this->session->set_flashdata("msg", "Information updated Succesfully.!!");
					redirect('Admin/appNodes');
				} else {
					$this->session->set_flashdata("error", "Something went wrong, please contact sytem developer!");
					redirect('Admin/appNodes');
				}
			}
		} else {
