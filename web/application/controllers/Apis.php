...
class Apis extends CI_Controller
{
    ...
    public function airqoChannelFeed()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $validate = $this->ApisModel->validateAPI($api);
            if ($validate == $this->ApisModel->stateOk()) {
                $json_url_lt = FORECAST_LINK.$channel;
                $ch_lt = curl_init();
                curl_setopt($ch_lt, CURLOPT_URL, $json_url_lt);
                curl_setopt($ch_lt, CURLOPT_RETURNTRANSFER, 1);
                $feeds_json = curl_exec($ch_lt);
    ...
    {   
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($validate == $this->ApisModel->stateOk()) {
                ...
                curl_setopt($ch, CURLOPT_URL, FORECAST_LINK.$channel."/" . $selected_time);
...
public function airqoPlace24Hours()
    ...
    // moved from assets/job folder
    /*
    * Description: update node details incase a node has been rename, removed, etc
    * Frequency: daily, every midnight
    */
    public function airqoJobUpdateNodeDetails(){
        $devices = array();
        $response = array();
        $this->ApisModel->init();
