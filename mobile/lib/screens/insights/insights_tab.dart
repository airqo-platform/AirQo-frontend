...
class _InsightsTabState extends State<InsightsTab> {
  ...
  String selectedMiniChart = DateFormat('yyyy-MM-dd').format(
    DateTime.now(),
  );
  ...
  void _updateTitleDateTime(List<charts.Series<Insights, String>> data) {
    ...
    final lastAvailableInsight = insights.reduce(
      (value, element) {
  ...
  void _updateUI(Insights insight) {
    setState(() => _selectedMeasurement = insight);

    if (widget.frequency == Frequency.daily) {
      setState(() => _lastUpdated = insight.empty
          ? 'Not available'
