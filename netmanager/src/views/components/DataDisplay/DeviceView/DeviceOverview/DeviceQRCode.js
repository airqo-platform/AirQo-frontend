...
const DeviceQRCode = ({ deviceData, fromDeviceData }) => {
  ...
  useEffect(() => {
    if (!isEmpty(deviceData)) {
      QRCodeApi({ id: deviceData._id, include_site: 'no' })
        .then((resData) => {
          setSrc(resData.data);
        })
        .catch((err) => {
  ...
  return (
    <ChartContainer title={'device QR Code'} green centerItems>
      ...
      >
        ...
        {src && <img alt="device qr code" src={src} height={'68%'} width={'auto'} />}
        {fromDeviceData && !isEmpty(deviceData) && (
          <QRCode
            value={JSON.stringify(pick(deviceData, 'name', 'long_name', 'latitude', 'longitude'))}
            renderAs="svg"
            size={256}
          />
        )}
