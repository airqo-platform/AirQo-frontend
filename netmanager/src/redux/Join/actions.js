...
export const forgotPassword = (userData) => (dispatch) => {
  ...
  axios
    ...
    .then((response) => {
      ...
      if (response.data.email === "Email does not exist") {
        dispatch({
          type: RECOVERY_EMAIL_FAILED,
        });
      } else if (response.data.email === "unable to send email") {
        dispatch({
          type: RECOVERY_EMAIL_FAILED,
        });
      } else if (response.data.email === "recovery email sent") {
        dispatch({
...
export const fetchDefaults = (userId) => {
  return (dispatch) => {
    ...
    return fetch(constants.DEFAULTS_URI + "/" + `${userId}`).then(
      (response) => {
        if (response.ok) {
          response.json().then((data) => {
