...
export const fetchCandidates = (id) => {
  return (dispatch) => {
    ...
    return axios
      ...
      .then((response) => response.data)
      .then((data) =>
        dispatch(fetchCandidatesSuccess(data.candidates, data.message)),
      )
      ...
      .catch((err) => {
        if (err.response) {
          ...
          dispatch({
            type: GET_ERRORS,
            payload: {
              data: { message: 'Please check your internet connectivity' },
            },
          });
        } else throw err();
      })
...
export const updatePassword = (userData) => (dispatch, getState) => {
  ...
  axios
    ...
    .then((response) => response.data)
    .then((data) =>
      dispatch({ type: UPDATE_PASSWORD_SUCCESS, payload: data.result }),
    )
    .catch((error) =>
      dispatch({ type: GET_ERRORS, payload: error.response || null }),
    );
};
