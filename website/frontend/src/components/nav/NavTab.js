...
const NavTab = ({ text, width, hideArrow, colored, filled, style, path, onClick, externalLink }) => {
    return(
        <>
        {hideArrow ? (externalLink ? (<a href={path} className={classNames("NavTab", { filled })} style={{ textDecoration:"none", color:"#fff", width: width - 32, ...(style || {}) }} onClick={onClick}>{text}</a>) : (<Link to={path || "/"} style={{textDecoration:"none", color:"#000"}}>
